...
if ($NATS_Session->userlevel<5) UL_Error("Edit View");
...
if (isset($_REQUEST['action']))
	...
	switch ($_REQUEST['action'])
		...
		case "save_edit":
			...
			$NATS->DB->Query($q);
			if ($NATS->DB->Affected_Rows()<=0) $msg="Failed to update or nothing changed";
			else $msg="Saved changes ok";
			break;
		...
		case "del_item":
			...
			$NATS->DB->Query($q);
			if ($NATS->DB->Affected_Rows()>0) $msg="Deleted Item";
			else $msg="Delete Item Failed";
			break;
...
ob_end_flush();
Screen_Header("View Editor",1,1,"","main","views");
...
echo "<b class=\"subtitle\">Editing View: <a href=view.php?viewid=".$_REQUEST['viewid'].">".$row['vtitle']."</a></b>";
...
//echo "</td></tr></table>";
echo " <b style=\"font-size: 13pt;\">[ <a href=\"view.php?viewid=".$_REQUEST['viewid']."\" target=top>Preview</a> | ";
echo "<a href=view.link.php?viewid=".$_REQUEST['viewid'].">Linking</a> ]</b>";
echo "<br>";
...
if (!isset($_REQUEST['show_options']))
 echo "document.getElementById('view_edit_options').innerHTML=\"<b>[ <a href=\\\"javascript:show_edit_options()\\\">Expand View Options</a> ]</b>\";\n";
echo "</script>\n";
