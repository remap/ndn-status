...
if ($NATS_Session->userlevel<1) UL_Error("View Node");
ob_end_flush();
Screen_Header("Viewing Node ".$_REQUEST['nodeid'],1);
?>
<br>
<?php
...
if ($arow=$NATS->DB->Fetch_Array($ar))
	{
	echo "<a href=history.alert.php?nodeid=".$_REQUEST['nodeid']."><b class=\"al".$arow['alertlevel']."\">Yes - ".oText($arow['alertlevel'])."</b></a> ";
	echo "(Opened ".dtago($arow['openedx']).")";
	}
else echo "<b>No</b> [ <a href=history.alert.php?nodeid=".$_REQUEST['nodeid'].">Alert History</a> ]";
echo "</td></tr>";

if ($NATS_Session->userlevel>4) echo "<tr><td align=right>Edit :</td><td align=left><a href=node.edit.php?nodeid=".$_REQUEST['nodeid'].">Edit Node Options</a></td></tr>";

echo "<tr><td align=right>Summary :</td><td align=left>";
$finishx=time();
$startx=$finishx-(60*60*24);
echo "<a href=summary.test.php?nodeid=".$_REQUEST['nodeid']."&startx=".$startx."&finishx=".$finishx.">Last 24hrs</a> | ";
echo "<a href=summary.test.php?nodeid=".$_REQUEST['nodeid'].">Today</a>";
echo "</td></tr>";
...
for ($a=0; $a<3; $a++)
	...
	if ($row[$ulink]==1)
		...
		if (!$shown_header) // first one
			{
			echo "<tr><td align=right>Links :</td><td align=left>";
			$shown_header=true;
			}
		$url=$row[$uurl];
...
echo "<b class=\"subtitle\">Local Tests</b><br><br>";
...
while ($row=$NATS->DB->Fetch_Array($r))
	...
	if (($row['testrecord']==1)||($row['testtype']=="ICMP")) 
		echo "[ <a href=\"history.test.php?testid=L".$row['localtestid']."\">History</a> ]";
	else echo "&nbsp;";
	echo "</td>";
...
if ($node['nsenabled']==1)
	{
	echo "<b class=\"subtitle\">Nodeside Tests</b><br><br>";
	...
	while ($row=$NATS->DB->Fetch_Array($r))
		...
		echo "<td>";
		if (($row['testrecord']==1)) 
			echo "[ <a href=\"history.test.php?testid=N".$row['nstestid']."\">History</a> ]";
		else echo "&nbsp;";
		echo "</td>";
