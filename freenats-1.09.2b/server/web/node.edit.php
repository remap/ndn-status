...
if (isset($_REQUEST['action']))
	...
	if ($_REQUEST['action']=="save_edit")
		...
		$q="UPDATE fnnode SET ";
		$q.="nodename=\"".ss($_REQUEST['nodename'])."\",";
		$q.="nodedesc=\"".ss($_REQUEST['nodedesc'])."\",";
		...
		if (isset($_REQUEST['nsurl'])) $q.="nsurl=\"".ss($_REQUEST['nsurl'])."\",";
		if (isset($_REQUEST['nskey'])) $q.="nskey=\"".ss($_REQUEST['nskey'])."\",";
		...
		if ($NATS->DB->Affected_Rows()<=0) 
			{
			echo "<b style=\"color: red;\">Failed to Save Changes or Nothing Changed</b><br>";
			$poplist[]="Failed to Save or Nothing Changed";
			}
		else 
			{
			echo "<b style=\"color: green;\">Saved Changes</b><br>";
			$poplist[]="Saved Changes";
			}
		...
		if ($res===false) 
			{
			$poplist[]="Error Fetching Nodeside Data";
			echo "<b style=\"color: red;\">Error Fetching Nodeside Data</b><br><br>";
			}
		else 
			{
			$poplist[]="Nodeside Data Refreshed";
			echo "<b style=\"color: green;\">Refreshed Nodeside Data</b><br><br>";
			}
		}
		...
		echo "<b>Saved Alert List Preferences</b><br>";
		$poplist[]="Saved Alert List Preferences";
...
echo "<b class=\"subtitle\">Editing <a href=main.php?mode=nodes>Node</a>: <a href=node.php?nodeid=".$_REQUEST['nodeid'].">".$_REQUEST['nodeid']."</a></b><br><br>";
...
if (!$row=$NATS->DB->Fetch_Array($r))
	{
	echo "No such node.<br><br>";
	Screen_Footer();
	exit();
	}
$NATS->DB->Free($r);

$title="<b class=\"sectitle\">Node Settings</b>";
...
echo "</select> [ <a href=\"javascript:showIcons()\">Show/Hide Icons</a> ]";
echo "</td></tr>";
...
echo "<tr><td align=right>
Master Node :</td><td align=left>";
echo "<select name=masterid>\n";
if ($row['masterid']=="") $s=" selected";
else $s="";
echo "<option value=\"\"".$s.">No Master</option>\n";
...
echo "<tr><td align=right>
Skip if Master :</td><td align=left>";
echo "<select name=masterjustping>\n";

if ($row['masterjustping']==1) $s=" selected";
else $s="";

echo "<option value=1".$s.">Fails Ping</option>\n";
if ($s=="") $s=" selected";
else $s="";
echo "<option value=0".$s.">Fails Any Test</option>\n";
...
echo "Minutes ".hlink("Node:TestInterval");
echo "</td></tr>";
...
echo " <a href=node.edit.php?nodeid=".$_REQUEST['nodeid']."&action=invalidate>Check ASAP</a> ";
...
if ($nodeside)
	...
	echo "<tr><td align=right>Pull URL :</td><td align=left>";
	echo "<input type=text name=nsurl value=\"".$row['nsurl']."\" size=30 maxlength=250>";
	if ($row['nsurl']!="")
		{
		$uri=$row['nsurl']."?nodekey=".$row['nskey']."&noupdates=1";
		echo " <a href=\"".$uri."\" target=top><i>Debug Raw Output</i></a>";
		}
	echo " ".hlink("Nodeside:URL");
	echo "</td></tr>";
	
	echo "<tr><td align=right>Pull Interval :</td><td align=left>";
	echo "<input type=text name=nsinterval value=\"".$row['nsinterval']."\" size=2 maxlength=10>";
	echo " Minutes ".hlink("Nodeside:Interval");
	echo "</td></tr>";
	
	echo "<tr><td align=right>Push Enabled :</td>";
	...
	if ($nsrow=$NATS->DB->Fetch_Array($nsr))
		{
		echo $nsrow['COUNT(nstestid)']." Node-Side Monitors";
		}
	$NATS->DB->Free($nsr);
	echo "</td></tr>";
	
	echo "<tr><td align=right>Refresh Now :</td><td align=left>";
	if ($row['nspullenabled']==1) echo "<a href=node.edit.php?nodeid=".$_REQUEST['nodeid']."&action=nodesiderefresh>Refresh (Pull) Now</a>";
	else echo "<i>Pull Method Disabled</i>";
	echo "</td></tr>";
...
//{ 
echo "<tr><td align=right valign=top>Utility Links ".hlink("UtilLinks",12)." :</td><td align=left>";
echo "<table width=100% border=0 cellspacing=0 cellpadding=0>";
echo "<tr><td>&nbsp;</td><td>Title</td><td>URL </td></tr>";
...
echo "<tr><td colspan=2>";
echo "<input type=submit value=\"Save Node Settings\"> <a href=main.php>Cancel Changes</a>";
echo "<br><br>";
echo "</td></tr>";
echo "</table>";
echo "</div></form>";
echo "<i><a href=\"javascript:showOptions()\">Show / Hide Node Options</a></i>";
...
echo "<tr><td><b>Add Test :</b></td>";
echo "<td><select name=testtype>";
echo "<option value=wtime>Web Time</option>";
//echo "<option value=tcp>TCP Port</option>";
echo "<option value=wsize>Web Size</option><option value=ping>Remote Ping</option>";
//echo "<option value=imap>IMAP Connection</option><option value=smtp>SMTP Connection</option>";
//echo "<option value=mysql>MySQL Connect Time</option><option value=mysqlrows>MySQL Row Count</option>";
echo "<option value=host>Simple DNS Host</option>";
//echo "<option value=dns>DNS Query</option>";
// New Test Manager List
foreach($NATS->Tests->QuickList as $key => $val)
	{
	echo "<option value=".$key.">".$val."</option>";
	}
echo "<option value=testrand>Test Random</option><option value=testloop>Test Loop</option>";
echo "</select> ";
...
if ($nodeside)
	...
	echo "<input type=hidden name=nodeid value=".$_REQUEST['nodeid'].">";
	echo "<tr><td><b>Add Node-side Monitor :</b></td>";
...
$title="<b class=\"sectitle\">Alert Actions</b> ".hlink("AlertAction",12);
Start_Round($title,600);
...
$NATS->DB->Free($r);
echo "<tr><td colspan=\"2\"><input type=submit value=\"Update Alert Actions\"></td></tr>";
echo "</form></table>";
End_Round();
?>
<br><br>
